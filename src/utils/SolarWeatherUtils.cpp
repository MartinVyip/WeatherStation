...
void adjustSolarEvents(state_config& state, uint8_t month, uint8_t day, uint8_t weekday) {
    ...
    state.sunrise = (solar_noon - (daylight_minutes >> 1));
    state.sunset = (solar_noon + (daylight_minutes >> 1));
}
...
void adjustDaytime(state_config& state, uint8_t hour, uint8_t minute) {
    ...
    state.daytime = (curr_minutes >= state.sunrise && curr_minutes < state.sunset);
}